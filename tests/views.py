...
def submit_test(request, test_id):
    ...
    for question in questions:
        ...
        user_answer, is_correct = None, False
        if answer_id:
            try:
    ...
    for qd in questions_details:
        StudentAnswer.objects.create(
            result=result, question=qd['question'],
            selected_answer=qd['user_answer'], is_correct=qd['is_correct']
        )
...
def test_result(request, result_id):
    ...
    student_name = f"{result.first_name} {result.last_name}" if result.first_name else (result.student.username if result.student else "Аноним")
    ...
    return render(request, 'tests/test_result.html', {
        'result': result, 'test': result.test, 'score': result.score,
        'total': result.total, 'percent': percent,
        'student_name': student_name, 'questions_details': questions_details,
    })
...
def teacher_home(request):
    ...
    return render(request, 'tests/teacher_home.html', {
        ...
        'total_tests': paginator.count, 'all_tags': Tag.objects.all(),
...
def test_results(request, test_id):
    ...
    students = {}
    for r in results:
        name = f"{r.first_name} {r.last_name}" if r.first_name else (r.student.username if r.student else "Аноним")
        students.setdefault(name, []).append({
            'id': r.id, 'score': r.score, 'total': r.total,
            'percent': int(r.score / r.total * 100) if r.total > 0 else 0,
            'completed_at': r.completed_at,
        })
    return render(request, 'tests/test_results.html', {'test': test, 'students': students})
...
def result_detail(request, result_id):
    ...
    for sa in student_answers:
        ...
        questions_data.append({
            'question': sa.question, 'user_answer': sa.selected_answer,
            'is_correct': sa.is_correct, 'correct_answer': correct_answer,
        })
    return render(request, 'tests/result_detail.html', {
        'result': result, 'student_name': student_name,
        'percent': percent, 'questions_data': questions_data,
    })
...
def users_list(request):
    ...
    return render(request, 'tests/users_list.html', {'users': users, 'current_role': role_filter})
...
def create_test(request):
    ...
    if request.method == 'POST':
        ...
        form = CreateTestForm()
    return render(request, 'tests/create_test.html', {'form': form, 'all_tags': Tag.objects.all()})
...
def test_questions(request, test_id):
    ...
    return render(request, 'tests/test_questions.html', {'test': test, 'questions': questions})
...
def edit_test(request, test_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('test_questions', test_id=test.id)
    return render(request, 'tests/edit_test.html', {'test': test, 'all_tags': Tag.objects.all()})
...
def edit_question(request, question_id):
    ...
    correct_index = next((i for i, ans in enumerate(answers, 1) if ans.is_correct), None)
    return render(request, 'tests/edit_question.html', {
        'question': question, 'answers': answers,
        'num_answers': num_answers, 'correct_index': correct_index, 'test': test,
    })
...
def admin_panel(request):
    ...
    for inv in invitations:
        invitations_with_urls.append({
            ...
            'used': inv.used, 'created_at': inv.created_at,
